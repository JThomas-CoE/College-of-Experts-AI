...
# CoE-Qwen3.5-Web-Development-18B-A3B — College of Experts Specialist · Beta
...
**Domain:** Web Development — frontend, HTML/CSS/JS, web frameworks, UI
...
The retained expert indices for this model are in `masks/coverage_WEB_T_K128.pt`
...
```bash
# Pull and run
ollama pull JThomas-CoE/coe-qwen3.5-web-18b-a3b

ollama run JThomas-CoE/coe-qwen3.5-web-18b-a3b
```
...
ollama create coe-qwen3.5-web-18b-a3b -f Modelfile
ollama run coe-qwen3.5-web-18b-a3b
```
...
The file `masks/coverage_WEB_T_K128.pt` in this repo contains the retained expert
indices used to produce this GGUF. Format:

```python
import torch
masks = torch.load("masks/coverage_WEB_T_K128.pt", weights_only=False)
...
python scripts/prune_gguf_from_mask.py \
    --mask   "masks/coverage_WEB_T_K128.pt" \
