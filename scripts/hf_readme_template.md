...
# CoE-Qwen3.5-{DOMAIN_FULL}-18B-A3B — College of Experts Specialist · Beta
...
**Domain:** {DOMAIN_FULL} — {DOMAIN_DESC}
...
The retained expert indices for this model are in `masks/coverage_{CORPUS_TAG}_K128.pt`
...
```bash
# Pull and run
ollama pull JThomas-CoE/coe-qwen3.5-{domain}-18b-a3b

ollama run JThomas-CoE/coe-qwen3.5-{domain}-18b-a3b
```
...
ollama create coe-qwen3.5-{domain}-18b-a3b -f Modelfile
ollama run coe-qwen3.5-{domain}-18b-a3b
```
...
The file `masks/coverage_{CORPUS_TAG}_K128.pt` in this repo contains the retained expert
indices used to produce this GGUF. Format:

```python
import torch
masks = torch.load("masks/coverage_{CORPUS_TAG}_K128.pt", weights_only=False)
...
python scripts/prune_gguf_from_mask.py \
    --mask   "masks/coverage_{CORPUS_TAG}_K128.pt" \
...
*[GitHub](https://github.com/JThomas-CoE/College-of-Experts-AI) · [Research Log](https://github.com/JThomas-CoE/College-of-Experts-AI)*
